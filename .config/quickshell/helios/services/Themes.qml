...
QtObject {
    ...
    function writeSystemThemeFiles() {
        ...
        try { root.writeKittyTheme(p); } catch (e) { console.warn("Helios theme: kitty failed", e); }
        ...
        try { root.writeYaziTheme(p); } catch (e) { console.warn("Helios theme: yazi failed", e); }
        try { root.writeAlacrittyTheme(p); } catch (e) { console.warn("Helios theme: alacritty failed", e); }
        try { root.writeWeztermTheme(p); } catch (e) { console.warn("Helios theme: wezterm failed", e); }
        try { root.writeTmuxTheme(p); } catch (e) { console.warn("Helios theme: tmux failed", e); }
        try { root.writeVscodeTheme(p); } catch (e) { console.warn("Helios theme: vscode failed", e); }
        try { root.writeKiroTheme(p); } catch (e) { console.warn("Helios theme: kiro failed", e); }
        try { root.writeRofiTheme(p); } catch (e) { console.warn("Helios theme: rofi failed", e); }
        try { root.writeWofiTheme(p); } catch (e) { console.warn("Helios theme: wofi failed", e); }
    }
    ...
    // mergeZedSettings, only replacing the "workbench.colorCustomizations"
    // key so the rest of the user's settings survive untouched.
    ...
    // Kiro (Amazon's VS Code fork) uses the same User/settings.json shape
    // and honors the same "workbench.colorCustomizations" key, so it just
    // reuses the VS Code builder/merger against its own config directory.
    function writeKiroTheme(p) {
        kiroSettingsFile.path = Quickshell.env("HOME") + "/.config/Kiro/User/settings.json";
        kiroSettingsFile.setText(root.mergeVscodeSettings(kiroSettingsFile.text(), root.buildVscodeColors(p)));
    }
